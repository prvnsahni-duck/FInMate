...
import { IsArray, IsDateString, IsIn, IsInt, IsNotEmpty, IsNumber, IsOptional, IsString, IsUUID, Matches, MaxLength, Min, Validate } from 'class-validator';
...
export class UpdateExpenseDto {
	...
	@Matches(/^[A-Z]{3}$/, { message: 'currency must be a valid ISO-4217 code (3 uppercase letters)' })
	@IsOptional()
	currency?: string;
	...
	@IsString()
	@Matches(/^\d{4}-\d{2}-\d{2}$/, { message: 'expenseDate must use YYYY-MM-DD format' })
	@IsDateString({}, { message: 'expenseDate must be a valid calendar date' })
	@IsOptional()
	expenseDate?: string;

	@IsIn(['draft', 'posted', 'void'], { message: 'status must be one of: draft, posted, void' })
	@IsOptional()
