...
async function main() {
  ...
  const mkExpense = (
    ...
    payer: GroupMember,
    total: number,
    txnType: 'expense' | 'refund' = 'expense',
    currency = 'USD',
  ) =>
    expRepo.save(
      expRepo.create({
  ...
  const net = async (a: string, b: string) =>
    (await people.getPersonDetail(a, b)).netBalance;
  ...
  console.log('\n[UAT 1-3] Direct lend + partial/full settlement…');
  {
    ...
    await people.createDirectTransaction(a.id, b.id, {
      entryType: 'lend',
      amount: 1,
      currency: 'USD',
    ...
    await people.createDirectSettlement(a.id, b.id, {
      amount: 2,
      currency: 'USD',
    ...
    check(
      'history has 3 lines (lend1, lend3, settle2) — originals intact',
      d.history.length === 3 &&
        d.history.filter((h) => h.source === 'direct').length === 2 &&
        d.history.filter((h) => h.source === 'settlement').length === 1,
      d.history.map((h) => `${h.source}:${h.amount}`),
    );
    ...
    check(
      'after full return ₹2: settled (₹0)',
      approx(await net(a.id, b.id), 0),
    );
    d = await people.getPersonDetail(a.id, b.id);
    check('direction is settled at ₹0', d.direction === 'settled');
    check(
      'history preserved after full settlement (4 lines)',
      d.history.length === 4,
      d.history.length,
    );
  }
  ...
  console.log('\n[UAT 4] Reverse relationship (borrow)…');
  {
    ...
    check(
      'after borrow ₹500: A owes B (net −500)',
      approx(await net(a.id, b.id), -500),
    );
    ...
    check(
      'after settle ₹200: A owes B ₹300 (net −300)',
      approx(await net(a.id, b.id), -300),
    );
  }
  ...
  );
  {
    ...
    await mkSplit(e, ma, 2);
    await mkSplit(e, mb, 2);
    await mkSplit(e, mc, 2);
    await mkPayment(e, ma, 1);
    await mkPayment(e, mb, 2);
    await mkPayment(e, mc, 3);
  ...
  console.log('\n[UAT 6] Unequal shares (2/3/5), A pays ₹10…');
  {
    ...
    await mkSplit(e, ma, 2);
    await mkSplit(e, mb, 3);
    await mkSplit(e, mc, 5);
    await mkPayment(e, ma, 10);
    check('B owes A ₹3', approx(await net(a.id, b.id), 3));
    check('C owes A ₹5', approx(await net(a.id, c.id), 5));
    const owed =
      (await people.getPersonDetail(a.id, b.id)).netBalance +
      (await people.getPersonDetail(a.id, c.id)).netBalance;
  ...
  console.log('\n[UAT 7] Multi-payer (shares 2/3/5; pay 4/6/0)…');
  {
    ...
    await mkSplit(e, mb, 3);
    await mkSplit(e, mc, 5);
    await mkPayment(e, ma, 4);
    await mkPayment(e, mb, 6);
  ...
  console.log('\n[UAT 8] Household exclusion…');
  {
    const a = await mkUser('u8a');
    const b = await mkUser('u8b');
    ...
    await mkSplit(e, ma, 50);
    await mkSplit(e, mb, 50);
    await mkPayment(e, ma, 100);
    const d = await people.getPersonDetail(a.id, b.id);
    check('household creates NO P2P debt', approx(d.netBalance, 0));
    check(
      'household absent from People history',
      d.history.length === 0,
      d.history.length,
    );
  }
  ...
  {
    const a = await mkUser('u9a');
    const b = await mkUser('u9b');
    ...
    await mkSplit(r, ma, 20);
    await mkSplit(r, mb, 20);
    await mkPayment(r, ma, 40);
    check(
      'after refund: B owes A ₹30 (refund reduces debt)',
      approx(await net(a.id, b.id), 30),
    );
    ...
    const aBal = gb.overall.balances.find(
      (x: any) => x.userId === a.id,
    )?.netBalance;
  ...
  {
    const a = await mkUser('u10a');
    const b = await mkUser('u10b');
    ...
    const kv = await AppDataSource.getRepository(GroupKeyVersion).save(
      AppDataSource.getRepository(GroupKeyVersion).create({
        group: g,
        version: 1,
        status: 'ACTIVE',
      }),
    ...
    const e = await mkExpense(g, a, ma, 10);
    (e as any).groupKeyVersion = kv;
    await expRepo.save(e);
    await mkSplit(e, ma, 5);
    await mkSplit(e, mb, 5);
    await mkPayment(e, ma, 10);
    const d = await people.getPersonDetail(a.id, b.id);
    const line = d.history.find((h) => h.source === 'group_expense');
    check(
      'group history line carries groupId/groupName/expenseId',
      !!line &&
        line.groupId === g.id &&
        !!line.groupName &&
        line.expenseId === e.id,
    ...
    check(
      'group history line carries decryption hints (scope + keyVersion)',
      !!line && line.encryptionScope === 'group' && !!line.groupKeyVersionId,
      line,
    );
    check(
      'title is passed through as ciphertext for client-side decryption',
      line?.title === 'cipher',
    );
  }
  ...
  console.log('\n[UAT 11/15] Overview totals + currency safety…');
  {
    ...
    await people.createDirectTransaction(me.id, usdFriend.id, {
      entryType: 'lend',
      amount: 300,
      currency: 'USD',
      occurredOn: '2026-08-01',
    });
    await people.createDirectTransaction(me.id, inrFriend.id, {
      entryType: 'borrow',
      amount: 50,
      currency: 'INR',
      occurredOn: '2026-08-01',
    ...
    check(
      'hasMultipleCurrencies flagged',
      ov.hasMultipleCurrencies === true,
      ov,
    );
    check(
      'dominant currency is USD (largest activity)',
      ov.currency === 'USD',
      ov.currency,
    );
    check(
      'totals are single-currency (USD), not a mixed sum',
      approx(ov.totalYouAreOwed, 300) && approx(ov.totalYouOwe, 0),
      ov,
    );
    check(
      'both people listed with their own currency',
      ov.people.length === 2 &&
        ov.people.some((p) => p.currency === 'USD') &&
        ov.people.some((p) => p.currency === 'INR'),
      ov.people,
    );
