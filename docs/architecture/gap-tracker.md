...
| ATT-002 | Attachments | ZK model | Legacy `attachmentKeys` path stores plaintext filenames/storage keys (`expenses.service.ts:693-706`) — metadata leak vs ZK goal. *2026-07-17: implemented on `Expense-module0a`; row stays open pending ADR reconciliation.* | Done | High |
| NOTIF-001 | Notifications | ARCHITECTURE §2 rate limiting | Group-invite email endpoints have no throttle (`members.controller.ts:29`, `groups.controller.ts:39,122`) — email-bombing / enumeration vector. *2026-07-17: implemented on `Expense-module0a`; row stays open pending ADR reconciliation.* | Done | High |

## Medium

| ID | Module | Source-of-truth | Finding | Status | Priority |
|----|--------|-----------------|---------|--------|----------|
| ENC-002 | Encryption | ARCHITECTURE §2.7 / KI-1 | `GET /groups/:id/keys/me` ignores `?versionId=` and always serves ACTIVE; post-rotation history undecryptable after logout / for new members (`groups.controller.ts:342-352`, `groups.service.ts:1261-1291`). *2026-07-17: implemented on `Expense-module0a` — endpoint honors `?versionId=` (scoped, REVOKED rejected); row stays open pending ADR reconciliation.* | Done | Medium |
...
| EXP-002 | Expense | PROJECT_DECISIONS "reference key version used" | `RecurringExpense` encrypts title/desc with group key but has no `groupKeyVersionId` column (`recurring-expense.entity.ts:20-23`). *2026-07-17: implemented on `Expense-module0a`; row stays open pending ADR reconciliation.* | Done | Medium |
| EXP-003 | Expense | PROJECT_DECISIONS "reference key version used" | On update, `groupKeyVersion` is not refreshed to ACTIVE while FE re-encrypts with the active key → stored ciphertext/version mismatch after rotation (`expenses.service.ts:987-992`). *2026-07-17: implemented on `Expense-module0a` — client declares `groupKeyVersionId` on create/update; backend validates and stamps exactly that version (create had the same mismatch via rotation races). Row stays open pending ADR reconciliation.* | Done | Medium |
| EXP-004 | Expense | ARCHITECTURE §4.3 | `expense_splits` are hard-deleted on edit (no `@DeleteDateColumn`), contradicting "expenses and splits use soft-deletion" (`expense-split.entity.ts`, `expenses.service.ts:1008-1010`). *2026-07-17: implemented on `Expense-module0a`; row stays open pending ADR reconciliation.* | Done | Medium |
| EXP-005 | Expense | ARCHITECTURE §4.2 | `ExpenseSplit` has no `@VersionColumn`; `deleteExpense`/`restoreExpense` do no version check — stale-client races possible (`expenses.service.ts:1107-1216`). *2026-07-17: `@VersionColumn` added (migration 1719300…); delete/restore version checks still pending (API contract).* | In Progress | Medium |
...
| EXP-007 | Expense | ARCHITECTURE §5 (currency consistency) | Recurring group expenses skip the `currency == group.currency` validation applied to one-off expenses (`recurring-expenses.service.ts:219-291`). *2026-07-17: implemented on `Expense-module0a`; row stays open pending ADR reconciliation.* | Done | Medium |
| GRP-002 | Groups | ARCHITECTURE §6 audit logging | No audit log for `rotateGroupKey`, `provisionGroupKeys`, `createGroupInvite`, `regenerateInviteToken`, `updateContributions` — incl. security-sensitive key rotation. *2026-07-17: implemented on `Expense-module0a`; row stays open pending ADR reconciliation.* | Done | Medium |
