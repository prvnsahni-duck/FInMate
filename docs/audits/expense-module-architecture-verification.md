...
**🟠 Major work remaining vs the target v2 architecture; ZK foundation sound and non-duplicating.** Suitable to freeze the _current_ Expense architecture as a baseline, but the Verified Gaps (esp. G1/G2/G4/G5) and the ADR-dependent layers (projection/search/sync/attachments) must be resolved before v2 feature work is called complete.
