...
- `CreateNoteDto`/`UpdateNoteDto` correctly require ciphertext (`dto/note.dto.ts` `@IsCiphertext` on `title`/`body`) — the _design_ enforces ZK, but nothing consumes these DTOs.
...
- `auth.service.ts` only _reads_ `user.status !== 'active'` (`:103,:301`) for login gating; nothing sets status to deleted/deactivated or scrubs PII.
