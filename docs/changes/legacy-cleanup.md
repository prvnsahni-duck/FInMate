...
- **`direct_shared` expense plumbing** (entity/DTO/decryption surface) — the original audit (P2-5) explicitly separates this from the "Dead or Legacy Code" list: it is _dormant_, not dead. Backend validation actively rejects direct-shared non-group expenses (a live, reachable code path enforcing a business rule), unlike the six candidates above, which had zero callers at all. Not in this task's explicit review list; left untouched.
