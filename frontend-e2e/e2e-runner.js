...
function spawnCmd(cmd, args, env) {
  const p = spawn(cmd, args, { stdio: 'inherit', shell: true, env: Object.assign({}, process.env, env) });
  return p;
}
...
(async () => {
  try {
    ...
    const backend = spawnCmd('npx', ['nx', 'run', 'backend:serve'], { APP_ENV: 'e2e', NODE_ENV: 'e2e' });
    ...
    const frontend = spawnCmd('npx', ['nx', 'run', 'frontend:serve'], { APP_ENV: 'e2e', NODE_ENV: 'e2e' });
    ...
    const pw = spawnCmd('npx', ['playwright', 'test', '--reporter=list,html', '--output=dist/.playwright/frontend-e2e/test-output'], { APP_ENV: 'e2e', NODE_ENV: 'e2e' });
