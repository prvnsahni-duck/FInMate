...
export const test = baseTest.extend<DiagnosticsFixtures>({
  captureDiagnostics: [async ({ page, context }, use, testInfo) => {
    ...
    page.on('pageerror', error => pageErrors.push(`${new Date().toISOString()} ${String(error)}`));
    ...
    await testInfo.attach('failure screenshot', { path: screenshotPath, contentType: 'image/png' }).catch(() => undefined);

    const dom = await page.content().catch(() => '<dom unavailable>');
    await attachText(testInfo, out, 'dom.html', dom, 'text/html');
    await attachText(testInfo, out, 'console.txt', consoleLogs.join('\n'), 'text/plain');
    await attachText(testInfo, out, 'page-errors.txt', pageErrors.join('\n'), 'text/plain');
    await attachText(testInfo, out, 'network.json', JSON.stringify(network, null, 2), 'application/json');
    ...
    await attachText(testInfo, out, 'storage.json', storage, 'application/json');

    const cookies = await context.cookies().catch(() => []);
    await attachText(testInfo, out, 'cookies.json', JSON.stringify(cookies, null, 2), 'application/json');
...
async function attachText(testInfo: any, directory: string, name: string, body: string, contentType: string) {
  const filePath = path.join(directory, name);
  fs.writeFileSync(filePath, body);
  await testInfo.attach(name, { path: filePath, contentType }).catch(() => undefined);
}
