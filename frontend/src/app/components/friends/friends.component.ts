  ...
  template: `
    ...
    <div class="bg-white/70 dark:bg-finmate-card/70 backdrop-blur-xl border border-slate-200 dark:border-white/5 rounded-3xl p-6 shadow-xl shadow-black/5">
      ...
      <div *ngIf="isLoading" class="space-y-4">
        <div *ngFor="let i of [1, 2, 3]" class="h-20 bg-slate-200 dark:bg-white/5 rounded-2xl animate-pulse"></div>
      </div>

      <div *ngIf="!isLoading && friends.length === 0" class="text-center py-12">
      ...
      <div *ngIf="!isLoading && friends.length > 0" class="space-y-4">
        <div *ngFor="let friend of friends" class="border border-slate-100 dark:border-white/5 rounded-2xl overflow-hidden bg-slate-50/50 dark:bg-white/5 hover:border-finmate-neon/30 transition-all">
          ...
          <div *ngIf="friend.isExpanded" class="border-t border-slate-200/50 dark:border-white/5 bg-slate-100/50 dark:bg-black/10 px-6 py-4 space-y-3">
            <h5 class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Group Breakdown</h5>
            <div *ngFor="let det of friend.currencyDetails" class="flex justify-between items-center text-sm py-1.5 border-b border-dashed border-slate-200 dark:border-white/5 last:border-b-0">
...
export class FriendsComponent implements OnInit {
  ...
  fetchFriendsBalances() {
    this.http.get<FriendBalance[]>('/api/friends').subscribe({
      next: (res) => {
        this.friends = res;
