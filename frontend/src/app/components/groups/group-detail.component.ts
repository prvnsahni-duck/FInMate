  ...
  template: `
    ...
    <div *ngIf="!isLoading && group">
      <div class="flex items-center justify-between mb-8">
        ...
        <button class="py-2 px-4 bg-gradient-neon text-white rounded-xl font-semibold shadow-lg shadow-finmate-neon/30 hover:shadow-finmate-neon/50 transition-all flex items-center space-x-2">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div *ngIf="expenses.length > 0" class="space-y-4">
              <div *ngFor="let expense of expenses" class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 hover:border-finmate-neon/30 transition-colors">
                ...
                <div class="text-right">
                  ...
                  <p class="text-xs text-slate-500 dark:text-slate-400">Paid by you</p> <!-- Mock text -->
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-8">
          <div class="bg-white/70 dark:bg-finmate-card/70 backdrop-blur-xl border border-slate-200 dark:border-white/5 rounded-3xl p-6 shadow-xl shadow-black/5">
            ...
            <div class="text-3xl font-bold text-green-500 dark:text-green-400 mb-2">
              + $0.00
            </div>
            <p class="text-sm text-slate-500 dark:text-slate-400">You are owed by the group.</p>
            <button class="w-full mt-6 py-2 px-4 bg-slate-800 dark:bg-white/10 text-white rounded-xl font-semibold hover:bg-slate-700 dark:hover:bg-white/20 transition-all">
              Settle Up
            </button>
          </div>
        </div>
