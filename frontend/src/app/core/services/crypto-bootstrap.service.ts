...
export class CryptoBootstrapService {
  ...
  private async bootstrapUserKeys(email: string) {
    ...
    try {
      console.debug('[CryptoBootstrap] Ensuring user asymmetric keys exist for:', email);
      // Wait for master key to load or be derived
      const masterKey = await this.encryptionService.loadKeyFromSession(email);
      if (!masterKey) {
        console.debug('[CryptoBootstrap] Master key not derived/stored yet. Skipping bootstrap.');
      ...
      console.error('[CryptoBootstrap] Failed to bootstrap asymmetric keys:', err);
