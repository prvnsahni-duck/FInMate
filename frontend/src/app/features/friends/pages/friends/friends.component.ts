  ...
  template: `
    <div class="mb-8">
      ...
      <p class="text-slate-500 dark:text-slate-400 mt-2">
        See your net debts and credits aggregated across all groups.
      </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
      ...
      ></app-stats-card>
      <div
        class="bg-gradient-dark rounded-3xl p-6 shadow-xl shadow-black/10 text-white border border-white/10 relative overflow-hidden"
      >
        <div
          class="absolute top-0 right-0 w-32 h-32 bg-finmate-neon/20 rounded-full blur-[40px]"
        ></div>
        <h3 class="text-white/70 text-sm font-medium mb-1 relative z-10">
          Net Balance
        </h3>
        <p
          class="text-3xl font-bold relative z-10"
          [ngClass]="{
            'text-green-400': netTotal >= 0,
            'text-red-400': netTotal < 0,
          }"
        >
      ...
      @if (isLoading) {
        <div class="space-y-4">
          @for (i of [1, 2, 3]; track i) {
            <div
              class="h-20 bg-slate-200 dark:bg-white/5 rounded-2xl animate-pulse"
            ></div>
          }
        </div>
        ...
        <div class="text-center py-12">
          ...
          <p class="text-slate-500 dark:text-slate-400 font-medium">
            All settled up! No active debts or credits with friends.
          </p>
        </div>
      } @else {
        <div class="space-y-4">
            ...
            >
              ...
              >
                <div class="flex items-center space-x-4">
                  <div
                    class="w-12 h-12 rounded-full bg-slate-200 dark:bg-white/10 flex items-center justify-center font-bold text-slate-600 dark:text-slate-300"
                  >
                ...
                <div class="flex items-center space-x-6 text-right">
                  <div>
                    <p
                      ...
                      [ngClass]="{
                        'text-green-500 dark:text-green-400':
                          friend.netBalance > 0,
                        'text-red-500 dark:text-red-400': friend.netBalance < 0,
                        'text-slate-500': friend.netBalance === 0,
                      }"
                    >
                      {{
                        friend.netBalance > 0
                    ...
                    <p class="text-xs text-slate-400 dark:text-slate-500">
                      {{ friend.currencyDetails.length }} group{{
                        friend.currencyDetails.length > 1 ? 's' : ''
                      }}
                    </p>
                  </div>
                  <svg
                    class="w-5 h-5 text-slate-400 transition-transform duration-300"
                    [class.rotate-180]="friend.isExpanded"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              @if (friend.isExpanded) {
                <div
                  class="border-t border-slate-200/50 dark:border-white/5 bg-slate-100/50 dark:bg-black/10 px-6 py-4 space-y-3"
                >
                  <h5
                  ...
                  @for (det of friend.currencyDetails; track det.groupId) {
                    <div
                      class="flex justify-between items-center text-sm py-1.5 border-b border-dashed border-slate-200 dark:border-white/5 last:border-b-0"
                    >
                      <a
                        [routerLink]="['/groups', det.groupId]"
                        class="font-semibold text-finmate-neon2 hover:underline"
                      >
                        {{ det.groupName }}
                      </a>
                      <span
                        class="font-bold"
                        [ngClass]="{
                          'text-green-500 dark:text-green-400': det.amount > 0,
                          'text-red-500 dark:text-red-400': det.amount < 0,
                        }"
                      >
                        {{ det.amount > 0 ? '+' : ''
                        }}{{ det.amount | currency: det.currency }}
                      </span>
                    </div>
                  }
                </div>
