  ...
  template: `
    <div class="bg-white/70 dark:bg-finmate-card/70 backdrop-blur-xl border border-slate-200 dark:border-white/5 rounded-3xl p-6 shadow-xl shadow-black/5">
      ...
      <div class="text-3xl font-bold mb-2" [ngClass]="{'text-green-500 dark:text-green-400': userBalance() > 0, 'text-red-500 dark:text-red-400': userBalance() < 0, 'text-slate-500': userBalance() === 0}">
        {{ userBalance() > 0 ? '+' : '' }}{{ userBalance() | currency:groupCurrency() }}
      </div>
    ...
    @if (suggestedSettlements().length > 0) {
      <div class="bg-white/70 dark:bg-finmate-card/70 backdrop-blur-xl border border-slate-200 dark:border-white/5 rounded-3xl p-6 shadow-xl shadow-black/5 mt-8">
        <h2 class="text-xl font-bold mb-4">Suggested Settlements</h2>
        <div class="space-y-3">
          @for (s of suggestedSettlements(); track s.fromUserId + s.toUserId) {
            <div class="text-sm p-3 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5">
