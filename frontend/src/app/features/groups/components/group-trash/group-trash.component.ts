      ...
      @if (deletedExpenses().length === 0) {
        <div class="text-center py-12">
          <p class="text-slate-500 dark:text-slate-400">
            No deleted expenses found in this group.
          </p>
        </div>
      } @else {
        <div class="space-y-4">
          @for (item of deletedExpenses(); track item.id) {
            <div
              class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5"
            >
              <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                  Deleted: {{ item.deletedAt | date }} • Original:
                  {{ item.amountTotal | currency: groupCurrency() }}
                </p>
              </div>
              ...
              @if (!isViewer()) {
                <button
                  ...
                  class="py-1.5 px-3 bg-gradient-neon text-white rounded-xl text-xs font-semibold shadow hover:shadow-lg transition-all flex items-center space-x-1"
                >
                  <svg
                    class="w-3.5 h-3.5"
                    fill="none"
